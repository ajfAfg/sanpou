...
or_expr:
  ...
  | e1=or_expr op_t=OROR e2=and_expr
      { BinOp { loc = loc_of_pos $startpos; lhs = e1; op_t; op = Or; rhs = e2 } }

and_expr:
  | e=comparison_expr { e }
  | e1=and_expr op_t=ANDAND e2=comparison_expr
      { BinOp { loc = loc_of_pos $startpos; lhs = e1; op_t; op = And; rhs = e2 } }

comparison_expr:
  | e=add_expr { e }
  | e1=comparison_expr op_t=LT e2=add_expr
      { BinOp { loc = loc_of_pos $startpos; lhs = e1; op_t; op = Lt; rhs = e2 } }
  | e1=comparison_expr op_t=EQEQ e2=add_expr
      { BinOp { loc = loc_of_pos $startpos; lhs = e1; op_t; op = Eq; rhs = e2 } }
  | e1=comparison_expr op_t=NEQ e2=add_expr
      { BinOp { loc = loc_of_pos $startpos; lhs = e1; op_t; op = Neq; rhs = e2 } }
  | e1=comparison_expr op_t=LTEQ e2=add_expr
      { BinOp { loc = loc_of_pos $startpos; lhs = e1; op_t; op = LtEq; rhs = e2 } }
  | e1=comparison_expr op_t=GTEQ e2=add_expr
      { BinOp { loc = loc_of_pos $startpos; lhs = e1; op_t; op = GtEq; rhs = e2 } }

add_expr:
  | e=mult_expr { e }
  | e1=add_expr op_t=PLUS e2=mult_expr
      { BinOp { loc = loc_of_pos $startpos; lhs = e1; op_t; op = Plus; rhs = e2 } }
  | e1=add_expr op_t=MINUS e2=mult_expr
      { BinOp { loc = loc_of_pos $startpos; lhs = e1; op_t; op = Minus; rhs = e2 } }

mult_expr:
  | e=postfix_expr { e }
  | e1=mult_expr op_t=MULT e2=postfix_expr
      { BinOp { loc = loc_of_pos $startpos; lhs = e1; op_t; op = Mult; rhs = e2 } }

postfix_expr:
  | e=primary_expr { e }
  | lhs=postfix_expr lb_t=LBRACKET index=expr rb_t=RBRACKET
      { Subscript { loc = loc_of_pos $startpos; lhs; lb_t; index; rb_t } }

primary_expr:
  | op_t=MINUS rhs=primary_expr
      { UnOp { loc = loc_of_pos $startpos; op_t; op = Neg; rhs } }
  | i=INTV { let (t, value) = i in IntLit { loc = loc_of_pos $startpos; t; value } }
  | t=TRUE { BoolLit { loc = loc_of_pos $startpos; t; value = true } }
  | t=FALSE { BoolLit { loc = loc_of_pos $startpos; t; value = false } }
  | t=SELF { Self { loc = loc_of_pos $startpos; t } }
  | id=ID lp=LPAREN args=expr_list rp=RPAREN
      { let (name_t, name) = id in
        App { loc = loc_of_pos $startpos; name_t; name; lp; args; rp } }
  | id=ID { let (t, name) = id in Var { loc = loc_of_pos $startpos; t; name } }
  | lb=LBRACE id=ID in_t=IN lo=expr dotdot_t=DOTDOT hi=expr colon_t=COLON value=expr trailing_semi_t=option(SEMI) rb=RBRACE
      { let (binder_t, binder) = id in
        MapInit { loc = loc_of_pos $startpos; lb; binder_t; binder; in_t; lo;
                  dotdot_t; hi; colon_t; value; trailing_semi_t; rb } }
  | lp=LPAREN rp=RPAREN
      { Tuple { loc = loc_of_pos $startpos; lp; elems = empty_comma_list; trailing_comma = None; rp } }
  | lp=LPAREN e=expr c=COMMA rp=RPAREN
      { Tuple { loc = loc_of_pos $startpos; lp; elems = singleton_comma_list e; trailing_comma = Some c; rp } }
  | lp=LPAREN e1=expr c=COMMA rest=expr_list_nonempty rp=RPAREN
      { Tuple { loc = loc_of_pos $startpos; lp; elems = cons_comma_list c e1 rest; trailing_comma = None; rp } }
  | lb=LBRACKET rb=RBRACKET
      { Sequence { loc = loc_of_pos $startpos; lb; elems = empty_comma_list; trailing_comma = None; rb } }
  | lb=LBRACKET e=expr c=COMMA rb=RBRACKET
      { Sequence { loc = loc_of_pos $startpos; lb; elems = singleton_comma_list e; trailing_comma = Some c; rb } }
  | lb=LBRACKET e1=expr c=COMMA rest=expr_list_nonempty rb=RBRACKET
      { Sequence { loc = loc_of_pos $startpos; lb; elems = cons_comma_list c e1 rest; trailing_comma = None; rb } }
  | lb=LBRACKET e=expr rb=RBRACKET
      { Sequence { loc = loc_of_pos $startpos; lb; elems = singleton_comma_list e; trailing_comma = None; rb } }
  | lp=LPAREN inner=expr rp=RPAREN
      { Paren { loc = loc_of_pos $startpos; lp; inner; rp } }
